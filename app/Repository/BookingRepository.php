...
class BookingRepository extends BaseRepository
{
     ...
     * @param Job $model
     */
    function __construct(Job $model, MailerInterface $mailer, BookingService $bookingService, UsersServices $userServices)
    {
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!$cuser) throw new ModeNotFoundException("User not found");
        ...
        if (!$jobs)  throw new \ErrorException('Jobs not found');
    ...
    public function store($user, $data)
    {
        ...
        $response['message'] = 'Job created successsfully';
        $response['id'] = $job->id;
        return $response;
    }
    ...
    {
        return  $this->bookingService->store($post_data);
    }
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $allJobs = $this->userService->getSuperAdminUsers($requestdata);
        } else {
            $allJobs = $this->userService->getAdminUsers($requestdata);
        }
